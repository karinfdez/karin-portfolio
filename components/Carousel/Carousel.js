...
export default function Carousel({slidersPerView, carouselItems}) {
  return (
      <Swiper
        spaceBetween={30}
        className="flex w-[280px] w-[900px] md:w-[800px] xl:w-[2000px] h-[260px] md:h-[280px]"
        pagination={{
          clickable: true,
          dynamicBullets: true
        }}
        ...
        {carouselItems?.length > 0 && carouselItems.map(({name, image, url,id}) => (
          <SwiperSlide key={id} className="w-full">
              <div className="relative h-full">
                  <Image alt="project portfolio" width={400} height={200} className="object-cover rounded-[8px] h-full w-full" src={image[0]?.url}/>
